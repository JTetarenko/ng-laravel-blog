...
@section('content')
    ...
    <div class="col-md-8">
        ...
        @else
            ...
            <!-- Page Breadcrumb -->
            @if (isset($filter))
                @include('articles._breadcrumb', ['route' => 'filter authors'])
            @elseif(isset($search))
                @include('articles._breadcrumb', ['route' => 'search articles'])
            @else
                @include('articles._breadcrumb', ['route' => 'index'])
            @endif
                ...
                @if(Auth::check())
                    @if((Auth::user()->username === $article->user->username) or (Auth::user()->group_id === 1))
                        ...
                        {!! Form::open(['method' => 'delete', 'url' => 'articles/'. $article->slug, 'id' => $article->id]) !!}
                            ...
                            <!-- Article -->
                            @include('articles._article', ['article' => $article, 'route' => 'index'])
